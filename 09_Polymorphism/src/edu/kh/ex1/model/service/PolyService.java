...
public class PolyService {
	...
	public void ex1 () {
		...
		// Tesla 객체를 참조하느 변수의 타입이 Car(부모)이기 떄문에
		// Tesla 객체가 Car(부모) 객체로 변화함
		// -> 업 캐스팅
		
		Car car3 = new Spark(); // 오류 발생 안함
		// Spark 객체를 차조하는 변수의 타입이 Car(부모)이기 떄문에
		// Spark 객체가 Car(부모) 객체로 변화함
		// -> 업 캐스팅
		
		// 1) 자식 객체가 부모 객체로 변화였기 떄문에
		// 	자식만의 고유한 필드, 메서드를 사용할 수 없다.
		
		// 1-1) car (부모 = 부모)
		car .setEngline("V6 6기통 엔진");
		car.setFuel("휘발유");
		car.setWhell(4);
		// Car 본인의 메서드 모두 사용 가능
		
		// 1-2) tesla (자식 = 자식)
		tesla.setEngline("전기모터");
		tesla.setFuel("전기");
		tesla.setWhell(4);
		tesla.setBerryCapacitv(1000000);
		
		// Tesla 부모, 본인의 메서드 모두 사용 가능(일반적인 상속관계)
		
		// 1-3) car2 (부모 = 자식(Tesla))
		car2.setEngline("전기모터");
		car2.setFuel("전기");
		car2.setWhell(4);
		//car2.setBerryCapacitv(1000000);
		// The method setBerryCapacitv(int) is undefined for the type Car
		// -> Car로 인식중
		
		// car3 (부모 = 자식(Spark))
		car3.setEngline("경차엔진");
		car3.setFuel("휘발유");
		car3.setWhell(4);
		...
		// Tesla, Spark와 같은 자식 객체의 고유 필드, 메서드 사용불가
		// 컴파일러가 Car라고 인시갛고 있기 때문에..
		
		
		// 2) 다형성을 이용한 객체배열
		Car[] arr = new Car[3];
		// 각 요소Car 객체만 들어올 수 있는 객체 배열 선언 및 할당
		
		arr[0] =  car; // Car 주소
		
		arr[1] = car2; // Tesla 주소
		// Car 타입 참조변수 = 주소(Tesla) 객체 주소
		
		arr[2] = car3; // Spark 주소
		
		for(int i = 0; i< arr.length; i++) {
			System.out.println(i + "번쨰 인덱스의 엔진 : " + arr[i].getEngline());
		}
	...
	// 전달방은 Car 또는 자식객체(Tesla, Spark)의 엔진, 연로, 바퀴갯수를 출력하는 메서드
	public void printCar(Car temp) {
		...
		// 부모타입 참조변수 = 자식 액체
		// -> 다형성의 업캐스팅 모양과 똑같다.
		// -> Car와 Spark/ 상속관계이기 떄문에
		// temp이라는 Car타입(부모타입) 매개변수에 Carm Tesla, Spark의 주소는 참조가능하다!!!
		System.out.println("엔진 : " + temp.getEngline());
		System.out.println("연료 : " + temp.getFuel());
		System.out.println("바퀴 갯수 : " + temp.getWheel());
		System.out.println();
	}
	
	// 매개변수로 전달받은 정수 값에 따라 Car, Tesla, Spark 중 객체를 만들어 반환하는 메서드
	...
	public void ex2() {
		...
		// 다형성을 이용한 반환형 사용법
		// Car[] arr = {new Car(1), new Tesla(2), new Spark(3)}
		...
		// Tesla와 Spark는 같은 부모 Car를 갖고 있을 뿐 아무런 관계 아님
	...
	public void ex3() {
		...
		System.out.println( ((Tesla)c1).getBerryCapacitv() );
		// 주의!
		// "." 연산 (Tesla)형변환 연산자보다 우선순위 높음!
		// 그래서 형변화 부분을 () 로 한번 묵어주고 나서. 연산을 진행해야한다.
		// Telsa 객체의 getBatteryCapaciy() 를 사용할 수 있음
		
		Tesla t1 = (Tesla)c1; // 다운캐스팅
		System.out.println(t1.getBerryCapacitv());
	}
	...
	public void ex4() {
		...
		if(c1 instanceof Spark) { // c1이 Spark 이거나 상속관계인가?
			...
			System.out.println("실패. c1은 Spark 타입/종속관게 아님");
	...
	public void ex5() {
		// 바인딩(Binding)
		// 실제 실행할 메서드 코드와 호출하는 코드를 연결시키니는 것.
		
		Car c1 = new Car("경유엔진", "경유", 8);
		
		System.out.println(c1.toString());
		// String edu.kh.ex1.model.dto.Car.toString ()
		// 반환형 출처
		// Car 객체에 있는 toString() 메서드를 호출
		// 프로그램 " 실행 전"
		// 컴파일러는 toString() 메서드가 Car에 있는 걸로 인식해서
		// c1.toString() 호출코드와
		// String edu.kh.ex1.model.dto.Car.toString () 메서드 선언부 코드를 연결
		// -> 정적바인딩
