...
public class StudentService {
	...
	// java.util.Linked List : 추가, 수정, 삭제 효율적

	// <E> : 제네릭
	// 타입을 제한하는 문법
	// 제네릭은 객체(Reference Type)만 허용
	// Java 7 이상에서는 우변 제닉 타입을 생략 가능
	// 다이아몬드 연산자 : 컴파일러가 좌변 부분을 보고 우변의 타입을 추촌

	private List<Object> testList = new ArrayList<>();

	// 학생 정보를 저장할 List 생성
	private List<Student> studentList = new ArrayList<>(); // new ArrayList<>()까지 추가해줘야 한다.
	...
	// 하지만 리스트의 크기는 늘었따 줄었다 하기 때문에 큰 의미는 없음
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				// menuNum이 초기값인 0을 가지고 있어서 반목문이 종료됨
	...
	public String addStudent() {
		...
		// INpuMismatchException, NullPointerException..
		...
		/*
		 * Student std = new Student(name, age, region, gender, score); boolean result =
		 * studentList.add(std); if(result) {
		 * 
		 * } else {
		 * 
		 * }
	...
	public void selectAll() {
		...
		if (studentList.isEmpty()) {
			...
			return; // 현재 메서드를 정료하고 호출한곳으로 돌아감.
		}

		// 반복문을 이용하여 studentList에 있는 전체 학생 정보 출력
		// 일반 for문
		/*
		 * for(int i=0; i < studentList.size(); i++) { // length(X) System.out.println(
		 * studentList.get(i) ); }
		 */
	 ...
	 * 3. 학생 정보 수정 메서드
	 * 
	 * - 학생 정보가 studentList에 하나라도 있는지 검사, 없다면 "입력된 학생 정보가 없습니다" 문자열 반환
	 * 
	 * - 인덱스 번호로 선택하여 학생을 수정 입력된 숫자가 0보다 작은지 검사, 작다면 "음수는 입력할 수 없습니다." 반환
	 * 
	 * - studentList 범위 내 인덱스번호인지 검사, 이상이라면 "범위를 넘어선 값을 입력할 수 없는다" 반환
	 * 
	 * - 수정할 때
	...
	public String updateStudent() {
		...
		// 2) studentsList 범위 내 인덱스 번호인지 검사
