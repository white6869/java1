package edu.ke.io.pack2.model.service;
...
public class ByteService {
	...
	public void bufferedFileOutput() {
		 ...
		 * 
		 * BufferedOutputStream / BufferedInputStrea 출력용 보조 스트림 (바이트) / 입력용 보조 스트림(화이트)
		 * 
		 * - FileOutputStream은 1바이트씩 데이터를 입출력 -> 통로가 좁아서 데이터 입출력시 속도가 느림 -> 보조스트림 이용 시
		...
		try {
			...
			fos = new FileOutputStream("/io_test/20251014/바이트_테스트_buffered.txt");
	...
	public void fileByteInput() {
		...
		try {
			...
			// FileInputStream 이용 시 해당 경로에 반드시 실제 파일이 존재해야함!
			// -> FiLENotFoundException 발생시킴
			// : \io_test\20251014\노래가사.txt (지정된 파일을 찾을 수 없습니다.)
			
			// 방법1. 파일 내부 내용을 1byte씩 끝어서 가져오기
			// -> 2byte 범주의 글자들은 꺠지는 문제 발생함
			...
			while(true) {
				value = fis.read(); // 1byte씩 가져오기
				// 단, 더 이상 읽어올 값이 없으면 -1 반환
				
				if(value == -1) break; // 다 읽어봤으면 반복 멈춤
			...
			*/
			...
			// 방법 2. 파일에 저장된 모든 byte값을 다 읽어와 byte[] 형태로 반환하기
	...
	/**
	 * BufferedinputStrea 보조 스트림을 이용한 성능 향상
	 * 
	 * InputStream의 경우 뵈부 파일의 데이터를 읽어올 떄 사용
	 * ->읽어오는 파일 내부의 데이터 크기가 작으면
	 *  보조스트림의 성능 이점이 크게 눈에 띄지 않음.
	 */
	public void bufferedFileBtyteInput() {
	...
	public void fileCopy() {
		...
		BufferedReader br = null; // 무나졍ㄹ 입력용 스트림(Scanner 대신 사용)
		...
		try {
			...
			if( !file.exists() ) { // 해당 경로에 파일이 존재하지 않으면
				// 프린트 구문 출력
				System.out.println("[해당 경로에 파일지 존재하지 않습니다]");
